...
class StreamingrANS:
    ...
    Mirrored from the compressor implementation.
    """
    ...
    def encode(self, symbols):
        """
        Same encoder as in compressor.py (not used during decoding,
        but kept here so the two classes stay in sync).
        """
        state = self.L 
        bit_stream = [] 
        ...
        # We reverse here exactly like the compressor
        return state, bytearray(reversed(bit_stream))

    def decode(self, initial_state, stream, num_symbols):
        """
        Decoder that mirrors the compressor's StreamingrANS.decode.
        """
        state = initial_state
        ...
        for _ in range(num_symbols):
            ...
            # Decode state (inverse of the encoder's update step)
...
def decode_ans_file(path: str):
    """
    Reads .ans binary file generated by the compressor.
    ...
    # Decode: convert bytes -> list[int] for the iterator
